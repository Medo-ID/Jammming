...
export const getTracks = async (query) => {
  const endpoint = `${apiSpotify}search?q=${encodeURIComponent(
    query
  )}&type=track&limit=20`;
...
/**
 * Gets the current user's profile information including display name and profile image.
 * @returns {Object} - User profile data containing name, image URL, etc.
 */
